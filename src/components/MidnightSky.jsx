...
import { playStarChime, playShootingStarSound } from '../sound'
...
function getMidnightProgress() {
  ...
  if (dist >= 30) return 0.15 + 0.3 * (1 - (dist - 30) / 15)
  if (dist >= 15) return 0.45 + 0.3 * (1 - (dist - 15) / 15)
  if (dist >= 5) return 0.75 + 0.2 * (1 - (dist - 5) / 10)
...
export default function MidnightSky() {
  ...
  useEffect(() => {
    function tick() {
      ...
      setProgress(p)
      const r = Math.round(19 + (2 - 19) * p),
        g = Math.round(19 + (2 - 19) * p),
        b = Math.round(22 + (8 - 22) * p)
  ...
  const glow = 0.35 + progress * 0.3 + (pulsing ? 0.4 : 0)
  ...
  return (
    <>
      ...
      <div
        ...
        title="Klik bulan untuk menyalakan hujan bintang ✦"
      ...
      >
        <svg
          ...
          style={{
            ...
            filter: `drop-shadow(0 0 ${15 + glow * 35}px rgba(194,194,242,${glow})) drop-shadow(0 0 ${40 + glow * 50}px rgba(227,184,234,${glow * 0.6}))`,
          }}
        >
          <defs>
            <radialGradient id="mG" cx="40%" cy="40%">
              <stop offset="0%" stopColor="#fffde8" />
              <stop offset="60%" stopColor="#f0e9c8" />
              <stop offset="100%" stopColor="#d4cda6" />
            </radialGradient>
            <mask id="mM">
